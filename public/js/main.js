const galleryPhotos = [
  "foto1.jpg","foto2.jpg","foto3.jpg","foto4.jpg","foto5.jpg",
  "foto6.jpg","foto7.jpg","foto8.jpg","foto9.jpg","foto10.jpg",
  "foto11.jpg","foto12.jpg","foto13.jpg","foto14.jpg","foto15.jpg",
  "foto16.jpg","foto17.jpg","foto18.jpg","foto19.jpg","foto20.jpg",
  "foto21.jpg","foto22.jpg","foto23.jpg","foto24.jpg","foto25.jpg",
  "foto26.jpg","foto27.jpg","foto28.jpg","foto29.jpg","foto30.jpg",
  "foto31.jpg","foto32.jpg","foto33.jpg","foto34.jpg","foto35.jpg",
  "foto36.jpg","foto37.jpg","foto38.jpg","foto39.jpg","foto40.jpg",
  "foto41.jpg","foto42.jpg","foto43.jpg","foto44.jpg","foto45.jpg",
  "foto46.jpg","foto47.jpg","foto48.jpg","foto49.jpg","foto50.jpg",
  "foto51.jpg","foto52.jpg","foto53.jpg","foto54.jpg","foto55.jpg",
  "foto56.jpg","foto57.jpg","foto58.jpg","foto59.jpg","foto60.jpg",
  "foto61.jpg","foto62.jpg","foto63.jpg","foto64.jpg","foto65.jpg",
  "foto66.jpg",
].reverse();

// === 1) Widget galerie na hlavní stránce (BEM verze) ===
document.addEventListener("DOMContentLoaded", () => {
  const widgetGallery = document.querySelector(".home-widget__gallery");
  if (!widgetGallery) return;

  // vezmeme prvních 5
  galleryPhotos.slice(0, 5).forEach((img, i) => {
    const el = document.createElement("img");
    el.src = `foto/thumbnails/${img}`;
    el.alt = "Fotogalerie";
    el.loading = "lazy";
    el.classList.add("home-widget__photo");

    // index = 1..5, stejné číslování jako ve /fotogalerie
    el.addEventListener("click", () => {
      const index = i + 1;
      window.location.href = `/fotogalerie#${index}`;
    });

    widgetGallery.appendChild(el);
  });

  // === 2) Efekt záře při najetí (modifier class) ===
  widgetGallery.addEventListener("mouseover", (e) => {
    const target = e.target;
    if (target.classList.contains("home-widget__photo")) {
      target.classList.add("home-widget__photo--hovered");
    }
  });

  widgetGallery.addEventListener("mouseout", (e) => {
    const target = e.target;
    if (target.classList.contains("home-widget__photo")) {
      target.classList.remove("home-widget__photo--hovered");
    }
  });
});